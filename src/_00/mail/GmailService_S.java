...
import _00.mail.Interface.EmailService_S;
import _01_model._02_supply.Supply;

public class GmailService_S implements EmailService_S {
	private final Properties props = new Properties();
	private final String mailUser;
	private final String mailPassword;

	public GmailService_S(String mailUser, String mailPassword) {
	...
	@Override
	public void validationLink(Supply sup) throws AddressException, MessagingException, IOException {
		String link = String.format("http://localhost:8080/Presentation/welcome/verify?acc=%s&email=%s&mem=sup",
				sup.getSupply_acc(),sup.getSupply_email());

		String anchor = String.format("<a href='%s'>驗證郵件</a>", link);

		String html = String.format("請按 %s 啟用帳戶或複製鏈結至網址列：<br><br> %s", anchor, link);

		javax.mail.Message message = createMessage(mailUser, sup.getSupply_email(), "註冊結果", html);
	...
	@Override
	public void passwordResetLink(Supply sup) throws AddressException, MessagingException, IOException {
		String link = String.format("http://localhost:8080/Presentation/welcome/reset_password?acc=%s&email=%s&mem=sup",
				sup.getSupply_acc(), sup.getSupply_email());
		...
		javax.mail.Message message = createMessage(mailUser, sup.getSupply_email(), "重設密碼", html);
		Transport.send(message);
	}
}
